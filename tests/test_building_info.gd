...
func test_glossary_covers_every_effect_the_data_uses(t) -> void:
	...
	t.check(not inert.has("weapon_upgrade") and not inert.has("armor_upgrade"),
		"weapon and armour upgrades reach the battlefield now, so they are sold as live")
	t.check(described.has("drill"), "the drill grounds are explained too")
