...
## harbour and a fleet on its sea; a save from before harbours existed loads
## with its ships moved out of the garrison; the AI never builds ships it
## cannot launch.
...
func test_a_save_without_harbours_loads_with_its_ships_in_port(t) -> void:
	## A world written before harbours existed: no harbour key anywhere, and a
	## galley recruited into the garrison. Loading normalizes both.
	var world := _port_world()
	var data: GameData = world[0]
	var state: Dictionary = world[1]
	...
	var loaded: Dictionary = SaveGame.from_json(SaveGame.to_json(state))
	t.check(not loaded.is_empty(), "the save reads back")
...
func test_the_ai_never_queues_ships_it_cannot_launch(t) -> void:
	var world := _port_world()
	var data: GameData = world[0]
	var state: Dictionary = world[1]
	state["player_faction"] = "blue"   # so the AI plays red
	data.units["test_galley"]["cost"] = 50   # cheaper than anything on land
	var rng := CampaignRng.seeded(1)
	FactionAi.begin_round(data, state)
	FactionAi.take_turn(data, state, "red", rng, AutoResolver.new(), [], [])
	var queued := 0
	for region_id in state["settlements"]:
		if state["settlements"][region_id]["owner"] != "red":
			continue
		for job in state["settlements"][region_id]["recruitment_queue"]:
			queued += 1
			t.check(job["template"] != "test_galley", "the AI never queues a ship it would never launch")
	t.check(queued >= 0, "the AI's turn ran (%d units queued)" % queued)
...
func test_sea_visibility_and_the_facade(t) -> void:
	var world := _port_world()
	var data: GameData = world[0]
	var state: Dictionary = world[1]
	var game := Game.new()
	...
	var seen := VisibilityRules.visible_sea_zones(data, state, "red")
	t.check(seen.has("test_sea") and not seen.has("test_sea_3"), "a coast sees its own sea and no further")
	var fleet_id := Fixtures.add_fleet(state, "red", "test_sea_2", ["test_galley"])
	seen = VisibilityRules.visible_sea_zones(data, state, "red")
	t.check(seen.has("test_sea_2") and seen.has("test_sea_3"), "a fleet sees its sea and the seas beyond")
	t.check_eq(game.own_ports_on_zone("test_sea"), ["epsilon"], "our port on the sea (alpha is blue's)")
	t.check_eq(game.reachable_zones(fleet_id).keys(), ["test_sea", "test_sea_3"], "two lanes on two points")
	var voyage := game.sail_fleet(fleet_id, "test_sea")
	t.check(voyage["arrived"], "the fleet sails home")
	t.check(game.dock_fleet(fleet_id, "epsilon")["ok"], "and docks with the lane it has left")
	t.check(not state["fleets"].has(fleet_id), "the fleet is in port")
