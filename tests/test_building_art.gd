...
func test_every_building_level_resolves(t) -> void:
	...
	for level_id in data.building_levels:
		...
		resolved += 1
	t.check_eq(resolved, 312, "every authored building level has a picture")
...
func test_the_map_and_the_art_share_one_hash(t) -> void:
	## MapGeometry delegates to ArtNoise now. If the two ever drift the whole
	## landmass silently reshapes, so pin them together.
	for key in ["latium", "aegyptus", "roman_walls_3", ""]:
		for salt in [0, 1, 91, 65535]:
			t.check_near(MapGeometry._hash01(key, salt), ArtNoise.hash01(key, salt),
				0.0000001, "hash agrees for %s/%d" % [key, salt])
