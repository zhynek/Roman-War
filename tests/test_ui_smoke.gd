...
func test_battle_odds_and_the_storm_through_the_ui(t) -> void:
	## The military layer on screen: the assault button waits for the engines
	## and quotes the odds, the storm is confirmed with them, the battle report
	## reaches the log, and the knowledge scroll opens on the war record.
	var tree := Engine.get_main_loop() as SceneTree
	var game := Game.new_campaign("julii", 42)
	var screen := CampaignScreen.create(game)
	tree.root.add_child(screen)
	...
	t.check_eq(RegionPanel.odds_text({"ratio": 2.0, "attacker_win_chance": 0.999, "defender": {"soldiers": 100}}),
		"odds 2.00:1 — 99% to win", "a near-certainty never reads as certain")
	t.check_eq(RegionPanel.odds_text({"ratio": 20.0, "attacker_win_chance": 1.0, "defender": {"soldiers": 0}}),
		"undefended", "an empty garrison is named for what it is")

	# A julii army with a non-Roman neighbour to invest (Roman-on-Roman war is refused).
	var army_id := ""
	var target := ""
	var army_ids: Array = game.state["armies"].keys()
	army_ids.sort()
	for candidate in army_ids:
		...
		if target != "":
			break
	t.check(target != "", "a julii army stands next to a foreign city")
	if target == "":
		screen.free()
		return
	...
	if assault_button != null:
		t.check(assault_button.text.contains("odds") or assault_button.text.contains("undefended"),
			"the button quotes the odds")
		var dialogs_before := _confirmations(screen).size()
		assault_button.pressed.emit()
		var dialogs := _confirmations(screen)
		t.check(dialogs.size() == dialogs_before + 1, "the storm is confirmed first")
		if dialogs.size() > dialogs_before:
			var dialog: ConfirmationDialog = dialogs.back()
			t.check(dialog.dialog_text.contains("Storm the walls"), "and the dialog names the deed")
			var log_before := screen.report_log.get_parsed_text().length()
			dialog.confirmed.emit()
			var log_text := screen.report_log.get_parsed_text()
			t.check(log_text.length() > log_before and log_text.contains("Assault on"),
				"the battle report reaches the log")
			t.check(game.state["settlements"][target]["siege"] == null or game.state["settlements"][target]["owner"] == "julii",
				"the siege is decided one way or the other")

	# The knowledge scroll opens on the war record the warcraft techniques read.
	screen.knowledge_panel.open_for(game)
	var saw_record := false
	for child in screen.knowledge_panel._content.get_children():
		if child is Label and (child as Label).text.begins_with("War record"):
			saw_record = true
	t.check(saw_record, "the scroll shows the war record")
	screen.knowledge_panel.hide()
	screen.free()


func _panel_button(panel: Node, prefix: String) -> Button:
...
func _confirmations(screen: Node) -> Array:
	var found: Array = []
	for child in screen.get_children():
