...
func test_submission_only_when_crushed(t) -> void:
	...
	t.check(not DiplomacyRules.evaluate(data, state, demand)["accept"], "no one submits to an equal")
	var host: Array = []
	for i in range(20):
...
func test_tribute_streams(t) -> void:
	...
	t.check_eq(state["tributes"].size(), 1, "the tribute is recorded")
	var notices := DiplomacyRules.process_turn(data, state)
	t.check_eq(int(state["factions"]["red"]["treasury"]), 4500, "the first payment leaves")
	...
	t.check_eq(int(state["factions"]["red"]["treasury"]), 4500, "no more payments")
...
func test_dissolving_a_treaty_is_unilateral(t) -> void:
	...
	var result := DiplomacyRules.propose(data, state, {"from": "red", "to": "blue", "stance": "neutral"})
	t.check(result["accepted"], "leaving quietly needs no consent")
	t.check_eq(DiplomacyRules.stance_between(state, "red", "blue"), "neutral", "the alliance is over")
	t.check_near(DiplomacyRules.opinion(state, "blue", "red"), -100.0, 0.001, "and resented, to the floor")
