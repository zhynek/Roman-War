...
func test_transfer_conserves_movement_and_respects_cap(t) -> void:
	...
	t.check_eq(ForceRules.check_transfer_units(data, state, fresh, big, [0]), "over_cap", "twenty is the limit")
...
func test_facade_check_explains_refusals(t) -> void:
	...
	var garrison_size: int = game.state["settlements"][region]["garrison"].size()
	if garrison_size > 0:
		var raised := game.raise_army(region, [0])
