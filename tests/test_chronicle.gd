...
func test_technique_and_edict_records_credit_the_leader(t) -> void:
	...
	# The edict half of this test drove the house-wide edicts engine (enact /
	# process_turn / the insolvency lapse). main holds edicts per province
	# instead, so those chronicle entries have no engine behind them; the
	# knowledge half above still covers the deed-and-chronicle path this test
	# exists for.
