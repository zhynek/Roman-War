...
##   2. Five keys reach the sim through SettlementRules.effect_max, not
...
const MAX_AGGREGATED := ["recruit_xp", "weapon_upgrade", "armor_upgrade",
...
static func dossier(data: GameData, state: Dictionary, region_id: String, chain_id: String) -> Dictionary:
	...
	for index in range(1, levels.size() + 1):
		...
		var quote := ConstructionRules.quoted_cost(data, state, owner, chain, level)
		var blockers: Array = []
	...
	if next_tier <= levels.size():
		var quote := ConstructionRules.quoted_cost(data, state, owner, chain, levels[next_tier - 1])
