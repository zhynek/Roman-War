...
static func quoted_cost(data: GameData, state: Dictionary, faction_id: String,
	...
	## with.
	return {
		"cost": _discounted_cost(data, state, faction_id, chain, level),
		"build_turns": _discounted_turns(data, state, faction_id, level),
	}
...
static func available_projects(data: GameData, state: Dictionary, region_id: String) -> Array:
	...
	for chain in data.chains.values():
		...
		var quote := quoted_cost(data, state, settlement["owner"], chain, next_level)
		projects.append({
			"chain": chain["id"],
			"kind": chain["kind"],
