...
static func blockers_for(data: GameData, state: Dictionary, region_id: String,
	...
	# A chain may need another kind standing first (armouries need a barracks).
	var required_building: Dictionary = chain.get("requires_building", {})
	if not required_building.is_empty():
		var have := SettlementRules.building_tier(data, settlement, String(required_building["kind"]))
		if have < int(required_building["level"]):
			blockers.append({"kind": "requires_building", "params": {
				"kind_name": String(required_building["kind"]).replace("_", " "),
				"building_kind": String(required_building["kind"]),
				"tier": int(required_building["level"]), "have": have,
			}})
