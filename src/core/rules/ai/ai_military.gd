...
static func _assault(data: GameData, state: Dictionary, faction_id: String, army_id: String, region_id: String, rng: CampaignRng, resolver: BattleResolver, ai_notices: Array, character_notices: Array) -> void:
	...
	if result.get("captured", false):
		...
		# Read before the capture: afterwards the settlement answers to us, and
		# the day's dispatch wants to name who lost it.
		var previous_owner := String(state["settlements"][region_id]["owner"])
		...
		ai_notices.append({"kind": "captured", "faction": faction_id,
			"region": region_id, "previous_owner": previous_owner})
