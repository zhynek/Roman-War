...
func _draw_relief(ci: CanvasItem, geo: MapGeometry, id: String, surveyed: bool) -> void:
	...
	for glyph in geo.glyphs[id]:
		...
		match terrain:
			"mountains": ReliefGlyphs.mountain(ci, p, s, roll, shade, north)
			"forest": ReliefGlyphs.tree(ci, p, s, roll, shade)
			"hills": ReliefGlyphs.hill(ci, p, s, roll, shade)
			"desert": ReliefGlyphs.dune(ci, p, s, roll, shade)
			"marsh": ReliefGlyphs.reed(ci, p, s, roll, shade)
			"steppe": ReliefGlyphs.tuft(ci, p, s, roll, shade)
			_: ReliefGlyphs.furrow(ci, p, s, roll, shade)
