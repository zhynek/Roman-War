...
| 5 — Agents & diplomacy | Envoys/spies/assassins, negotiation offers, AI attitude model | **Done.** Agents as campaign entities (`AgentRules`, data/agents.json): envoys unlock the negotiation scroll on foreign soil, informers grant vision/open gates in assaults/risk the governor's spy-hunters (`agent_skill` wired), hired blades attempt murders against `personal_security`. Attitude model (`NegotiationRules`): structural + stored decaying ledger, named factor breakdowns; offers — peace (sweetened), trade, alliance, gifts, tribute demands, buying towns; war declarations scar the victim's ledger and steer AI target choice. Balance in `balance.json → agents/attitude` |
...
| 7 — Politics, events, victory | Full senate offices & mission variety, civil war depth, richer event scripting | **Done.** Office ladder (data/offices.json): summer elections fill every seat by house standing + personal gravitas, office effects flow through the character engine, `office_gained` triggers finally fire. Mission variety: all six authored kinds issued and judged (take_region, blockade_port — fleets have a purpose, make_alliance & reach_trade_agreement via Phase 5 negotiation, assassinate_leader via Phase 5 blades, leader_suicide with a comply path and outlawry on refusal). Civil war picks sides: low-standing houses join the rebellion, loyalists and the Senate close ranks. Expansion events (`faction_holds_regions` condition). Senate scroll UI |
| 8 — Polish | Campaign UI, balancing pass, tutorial, save robustness | **Done to overnight depth.** Visual identity built in code (`UiTheme`: ink/leather/bronze/gold across every control), full-window layout (long-standing anchors bug fixed), map with named seas at their charted anchors, shadowed tier-notched tokens, capital star, army banners with counts, agent marks; in-game Help scroll; projected net income in the top bar; households seeded for all 20 houses (91 spouses/children) so the family tree breathes from turn one; sim-driven balance via `tools/sim_campaign.gd` (living world across seeds/difficulties, no runaway debt, wars start and end); screenshot harness `tools/screenshot.gd`. Human playtest tuning remains |
